...
def augment_reaction(reaction):
    reag, kat = reaction.split('>')
    molecules = reag.split('.')
    ...
    result = ' . '.join(random_molecules) + " >" + kat
    return result
...
class AugSmiles(text_problems.Text2TextProblem):
    ...
    def generate_samples(self, data_dir, tmp_dir, dataset_split):
        ...
        for source, target in zip(source_smiles, target_smiles):
            try:
                for _ in range(2):
                    source = augment_reaction(source)
                    yield {
                        "inputs": source,
                        "targets": target,
                    }
